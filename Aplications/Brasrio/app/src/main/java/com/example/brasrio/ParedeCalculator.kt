...
class ParedeCalculator(
        ...
        fun calcularMateriais(): List<MaterialItem> {
            ...
            com.example.brasrio.CalculoUtils.addMaterialByCode("1516", ceil(fitaNecessaria / 90f).toInt(), materiaisSelecionados, produtos)
            ...
            if (massaNecessaria <= 5) {
                com.example.brasrio.CalculoUtils.addMaterialByCode("698", 1, materiaisSelecionados, produtos) // 5kg
            } else {
                val qtd28kg = ceil(massaNecessaria / 28f).toInt()
                com.example.brasrio.CalculoUtils.addMaterialByCode("431", qtd28kg, materiaisSelecionados, produtos) // 28kg
            }
            ...
            com.example.brasrio.CalculoUtils.addMaterialByCode("388", ceil(guiasNecessarias * 3f).toInt(), materiaisSelecionados, produtos) // Guia 48 (aumentada em 200%)
            com.example.brasrio.CalculoUtils.addMaterialByCode("387", ceil(montantesNecessarios * 1.1f).toInt(), materiaisSelecionados, produtos) // Montante 48 (aumentado em 10%)
